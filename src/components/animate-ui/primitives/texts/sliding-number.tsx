...
// Minimal local replacement for `react-use-measure` (height only), so this
// component stays dependency-free. Returns [refCallback, { height }].
function useMeasure(): [(el: HTMLElement | null) => void, { height: number }] {
...
function SlidingNumber({
  ...
  const baseIntLength = padStart
    ? Math.max(
        Math.floor(Math.abs(number)).toString().length,
        newIntStrRaw.length,
      )
    : newIntStrRaw.length;
  const finalIntLength = Math.max(minDigits, baseIntLength);

  const newIntStr = padStart || minDigits > 0
    ? newIntStrRaw.padStart(finalIntLength, '0')
    : newIntStrRaw;

  const prevFormatted = formatNumber(prevNumberRef.current);
  const [prevIntStrRaw = '', prevDecStrRaw = ''] = prevFormatted.split('.');
  const prevIntStr = padStart || minDigits > 0
